...
from utils.db import session, Contest as Contest_DB, Problem as Problem_DB, Submission as Submission_DB, Handle as Handle_DB
...
async def on_member_remove(event):
    q = session.query(Handle_DB)\
        .filter(Handle_DB.id == event.user_id)\
        .filter(Handle_DB.guild_id == event.guild_id)
    if q.count():
        q.delete()
        session.commit()
